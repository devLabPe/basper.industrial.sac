    ...
    $.fn.asHTML5VideoControls = function(skinFolder, parentInst, hidecontrols, hideplaybutton, defaultvolume) {
        ...
        if (hd) {
            ...
            $videoHD.css({
                ...
                parentInst.isHd = isHd; var isPaused = $videoObj.get(0).isPaused;
                $videoObj.get(0).setAttribute("src", (isHd ? hd : src) + "#t=" + $videoObj.get(0).currentTime); if (!isPaused) $videoObj.get(0).play();
        ...
        if ($videoObj.get(0).volume === volume / 2 + 0.1) {
            ...
            $videoVolume.css({
                ...
                height: BUTTON_SIZE + "px",
                margin: Math.floor((BAR_HEIGHT - BUTTON_SIZE) / 2)
            }).hover(function() { clearTimeout(hideVolumeBarTimeoutId); var volume = $videoObj.get(0).volume;
        ...
        var onVideoPlay = function() { if (!hideplaybutton) $videoPlay.hide(); if (!hidecontrols) { $videoBtnPlay.hide();
                $videoBtnPause.show() } };
        var onVideoPause = function() { if (!hideplaybutton) $videoPlay.show(); if (!hidecontrols) { $videoControls.show();
                clearTimeout(hideControlsTimerId);
                $videoBtnPlay.show();
                $videoBtnPause.hide() } };
        ...
        var onVideoUpdate = function() { var curTime = $videoObj.get(0).currentTime; if (curTime) { $videoTimeCurrent.text(calcTimeFormat(curTime)); var duration = $videoObj.get(0).duration; if (duration) { $videoTimeTotal.text(calcTimeFormat(duration)); if (!sliderDragging) { var sliderW = $videoSeeker.width(); var pos = Math.round(sliderW * curTime / duration);
                        $videoSeekerPlay.css({ width: pos });
                        $videoSeekerHandler.css({ left: pos }) } } } };
        var onVideoProgress = function() { if ($videoObj.get(0).buffered && $videoObj.get(0).buffered.length > 0 && !isNaN($videoObj.get(0).buffered.end(0)) && !isNaN($videoObj.get(0).duration)) { var sliderW = $videoSeeker.width();
...
(function($) {
    $.fn.ashtml5lightbox = function(options) {
        ...
        inst.createMarkup = function() {
            ...
            if (inst.options.navarrowsalwaysshowontouch || inst.options.alwaysshownavarrows) { styleCss += "#slider-html5-lightbox .html5-prev-touch {left:0px;top:50%;margin-top:-16px;margin-left:-32px;} #slider-html5-lightbox .html5-next-touch {right:0px;top:50%;margin-top:-16px;margin-right:-32px;}";
            ...
            if (inst.options.titlestyle == "right") { styleCss += "#slider-html5-lightbox #html5-elem-wrap {width:" + inst.options.imagepercentage + "%;height:100%;} #slider-html5-lightbox #html5-elem-data-box {min-height:100%;}";
                styleCss += "@media (max-width: " + inst.options.sidetobottomscreenwidth + "px) {#slider-html5-lightbox #html5-elem-wrap {width:100%;height:auto;} #slider-html5-lightbox #html5-elem-data-box {width:100%;height:auto;min-height:0;}}" } else if (inst.options.titlestyle == "left") {
        ...
        inst.calcNextPrevElem = function() {
            ...
            for (var i = 0; i < inst.elemArray.length; i++)
                if (inst.matchGroup(group,
                        inst.elemArray[i][ELEM_GROUP])) { if (i == inst.options.curElem) inst.options.groupIndex = inst.options.groupCount;
        ...
        inst.enableSwipe = function() { inst.$elem.sliderTouchSwipe({ preventWebBrowser: false, swipeLeft: function() { inst.gotoSlide(-1) }, swipeRight: function() { inst.gotoSlide(-2) } }) };
        ...
        inst.embedHTML5Video = function($container, src, autoplay, loopvideo) {
            ...
            $("video", $container).unbind("ended").bind("ended", function() { $(window).trigger("html5lightbox.videofinished"); if (inst.autosliding) inst.gotoSlide(-1);
                else if (inst.options.autoclose) inst.finish() })
        };
        ...
        inst.addItem =
            function(href, title, group, width, height, webm, ogg, thumbnail, description, mediatype) { type = mediatype && mediatype >= 0 ? mediatype : inst.checkType(href);
...
var ASPlatforms = {
    ...
    isIE9: function() { return navigator.userAgent.match(/MSIE 9/i) != null && !this.isOpera() },
    ...
    css33dTransformSupported: function() { return !this.isIE() && !this.isOpera() },
...
(function($) {
    $.fn.amazingslider = function(options) {
        ...
        var TYPE_IMAGE = 1,
            ...
            TYPE_IFRAME = 11;
        var AmazingSlider =
            function(container, options, textoptions, id) { this.container = container;
                ...
                $(".amazingslider-engine").css({ "display": "none" }); if (ASPlatforms.isMobile() && !this.options.navshowpreviewontouch) this.options.navshowpreview = false;
        ...
        AmazingSlider.prototype = {
            ...
            readTags: function() {
                ...
                $(".amazingslider-thumbnails", this.container).find("li").each(function(index) {
                    ...
                    if (img.length <= 0) img = $("img", $(this));
                    if (img.length > 0 && instance.elemArray.length > index) { var src = img.attr("src"); var lazytags = instance.options.lazyloadtags.split(","); for (var i = 0; i < lazytags.length; i++)
                            if (img.data(lazytags[i]) && img.data(lazytags[i]).length > 0) { src = img.data(lazytags[i]); break }
            ...
            init: function(instance) {
                ...
                if (instance.options.randomplay) {
                    for (i = elemLength - 1; i > 0; i--) { if (i == 1 && Math.random() < 0.5) break; var index = Math.floor(Math.random() * i); var temp = instance.elemArray[index];
            ...
            createRibbon: function() { if (!this.options.showribbon || this.options.ribbonimage.length <= 0) return;
            ...
            initVideoApi: function() {
                ...
                var initYoutube = false,
                    initVimeo = false;
                for (i = 0; i < this.elemArray.length; i++) { videos = this.elemArray[i][ELEM_VIDEO]; for (j = 0; j < videos.length; j++)
                ...
                if (initVimeo) { var tag = document.createElement("script");
                    tag.src = this.options.jsfolder + "froogaloop2.min.js"; var firstScriptTag = document.getElementsByTagName("script")[0];
                    firstScriptTag.parentNode.insertBefore(tag, firstScriptTag) }
            },
            createSliderTimeout: function() {
                var instance = this;
                this.sliderTimeout = new ASliderTimer(this.options.slideinterval, function() { instance.slideRun(-1) }, this.options.showtimer ?
                    function(percent) { instance.updateTimer(percent) } : null);
                if (instance.options.pauseonmouseover) $(".amazingslider-slider-" + this.id, this.container).hover(function() { if (!instance.isPaused) instance.sliderTimeout.pause() }, function() { if (!instance.isPaused) instance.sliderTimeout.resume(false) });
            ...
            updateTimer: function(percent) { w = Math.round(percent * 100); if (w > 100) w = 100; if (w < 0) w = 0;
            ...
            createMarkup: function() {
                ...
                var instance = this;
                if (this.options.enabletouchswipe) $(".amazingslider-swipe-box-" + this.id, this.container).sliderTouchSwipe({ preventWebBrowser: false, swipeLeft: function() { instance.slideRun(-1) }, swipeRight: function() { instance.slideRun(-2) } });
            ...
            embedHTML5Video: function($container, src, autoPlay) {
                ...
                var instance = this;
                $("video", $container).unbind("ended").bind("ended", function() { instance.videoPaused = false; if (!instance.isPaused) instance.slideRun(-1) })
            },
            ...
            playVimeoVideo: function(href, autoplay) {
                ...
                if (this.options.previewmode) { $videoWrapper.html("<div class='amazingslider-error-" + this.id + "'>To view Vimeo video, publish the slider then open it in your web browser</div>"); return } else {
                    ...
                    var instance = this;
                    vimeoPlayer.addEvent("ready", function() { vimeoPlayer.addEvent("finish", function(id) { instance.videoPaused = false; if (!instance.isPaused) instance.slideRun(-1) }) })
                }
            },
            playIframeVideo: function(href, autoplay) {
                ...
                $videoWrapper.css({ display: "block", width: "100%", height: "100%" });
                if (this.options.previewmode) { $videoWrapper.html("<div class='amazingslider-error-" + this.id + "'>To view the video, publish the slider then open it in your web browser</div>"); return } else { var src = href;
            ...
            getYoutubeParams: function(href) { var result = {}; if (href.indexOf("?") < 0) return result; var params = href.substring(href.indexOf("?") + 1).split("&"); for (var i = 0; i < params.length; i++) { var value = params[i].split("="); if (value && value.length == 2 && value[0].toLowerCase() != "v") result[value[0].toLowerCase()] = value[1] } return result },
            playYoutubeVideo: function(href, autoplay) {
                ...
                var instance = this;
                if (!ASYouTubeIframeAPIReady) { ASYouTubeTimeout += 100; if (ASYouTubeTimeout < 3E3) { setTimeout(function() { instance.playYoutubeVideo(href, autoplay) }, 100); return } }
                ...
                } else { var src = href + (href.indexOf("?") < 0 ? "?" : "&") + "autoplay=1&wmode=transparent&rel=0&autohide=1"; if (instance.options.videohidecontrols) src += "&controls=0&showinfo=0";
            ...
            hideTexteffect: function(event) { var sliderid = $(this).data("sliderid");
            ...
            createText: function() {
                ...
                this.container.bind("amazingslider.switchtext", function(event, prev, cur) {
                    $("head").find("style").each(function() { if ($(this).data("creator") == "amazingslidercreator" + instance.id && $(this).data("csstype") == "texteffect") $(this).remove() });
                    var textHeadCss =
                    ...
                    if (instance.textoptions.textstyle == "static") {
                        ...
                        switch (instance.textoptions.textpositionstatic) {
                            ...
                            case "bottom":
                                ...
                                break;
                            case "topoutside":
                                $textWrapper.css({ left: "0px", bottom: "100%", "margin-bottom": instance.textoptions.textpositionmarginstatic + "px" });
                                break;
                            case "bottomoutside":
                                $textWrapper.css({
                                    left: "0px",
                    ...
                    if (instance.textoptions.textstyle == "static") {
                        ...
                        if (!instance.elemArray[cur][ELEM_TITLE] && !instance.elemArray[cur][ELEM_DESCRIPTION] && !instance.elemArray[cur][ELEM_BUTTON]) $textWrapper.fadeOut();
                            ...
                            if (positions) { positions = positions.split(",");
                            ...
                            if (effects) { effects = effects.split(",");
                            ...
                            switch (effect) {
                                ...
                                case "slide":
                                    ...
                                    } else if (effect_dir == "right") { $textBox.css({ left: instance.textoptions.texteffectslidedistance + "px", opacity: 0 });
                                        $textBox.delay(instance.textoptions.texteffectdelay).animate({ left: "0px", opacity: 1 }, instance.textoptions.texteffectduration, instance.textoptions.texteffecteasing) } else if (effect_dir == "top") {
                                        $textBox.css({ top: "-" + instance.textoptions.texteffectslidedistance + "px", opacity: 0 });
                                        $textBox.delay(instance.textoptions.texteffectdelay).animate({
                                            top: "0px",
                                            opacity: 1
                                        }, instance.textoptions.texteffectduration, instance.textoptions.texteffecteasing)
                                    } else if (effect_dir == "bottom") { $textBox.css({ top: instance.textoptions.texteffectslidedistance + "px", opacity: 0 });
                            ...
                            if (instance.textoptions.texteffectseparate && instance.elemArray[cur][ELEM_DESCRIPTION].length >
                                ...
                                if (effects1) { effects1 = effects1.split(",");
                                ...
                                switch (effect1) {
                                    ...
                                    case "slide":
                                        ...
                                        if (effect_dir1 == "left") { $textBox1.css({ left: "-" + instance.textoptions.texteffectslidedistance1 + "px", opacity: 0 });
                                            $textBox1.delay(instance.textoptions.texteffectdelay1).animate({ left: "0px", opacity: 1 }, instance.textoptions.texteffectduration1, instance.textoptions.texteffecteasing1) } else if (effect_dir1 == "right") {
                                        ...
                                        } else if (effect_dir1 == "top") { $textBox1.css({ top: "-" + instance.textoptions.texteffectslidedistance1 + "px", opacity: 0 });
                                            $textBox1.delay(instance.textoptions.texteffectdelay1).animate({ top: "0px", opacity: 1 }, instance.textoptions.texteffectduration1, instance.textoptions.texteffecteasing1) } else if (effect_dir1 == "bottom") {
                            ...
                            if (instance.textoptions.texteffectseparate && instance.elemArray[cur][ELEM_BUTTON].length > 0) {
                                ...
                                if (effects2) { effects2 = effects2.split(",");
                                ...
                                switch (effect2) {
                                    ...
                                    case "slide":
                                        ...
                                        } else if (effect_dir2 == "right") { $textBox2.css({ left: instance.textoptions.texteffectslidedistance2 + "px", opacity: 0 });
                                            $textBox2.delay(instance.textoptions.texteffectdelay2).animate({ left: "0px", opacity: 1 }, instance.textoptions.texteffectduration2, instance.textoptions.texteffecteasing2) } else if (effect_dir2 == "top") {
                                            $textBox2.css({ top: "-" + instance.textoptions.texteffectslidedistance2 + "px", opacity: 0 });
                                            $textBox2.delay(instance.textoptions.texteffectdelay2).animate({
                                                top: "0px",
                                                opacity: 1
                                            }, instance.textoptions.texteffectduration2, instance.textoptions.texteffecteasing2)
                                        } else if (effect_dir2 == "bottom") { $textBox2.css({ top: instance.textoptions.texteffectslidedistance2 + "px", opacity: 0 });
            ...
            resizeFullscreenImage: function($imgElem) {
                ...
                if (sliderRatio < imgRatio)
                    if (this.options.scalemode ==
                        "fit") { this.curImageMaxWidth = sliderRatio / imgRatio * 100;
                        this.curImageMarginLeft = 50 - this.curImageMaxWidth / 2 } else this.curImageMarginTop = -1 * (imgRatio - sliderRatio) * 50;
                else if (this.options.scalemode == "fit") this.curImageMarginTop = (sliderRatio - imgRatio) * 50;
                else { this.curImageMarginLeft = -1 * (1 / imgRatio - 1 / sliderRatio) * 50;
            ...
            resizeSlider: function() {
                ...
                if (this.options.navdirection == "vertical") {
                    ...
                    if (this.options.navshowbuttons) {
                        if (this.options.navshowarrow) { len += len > 0 ? this.options.navspacing : 0;
                            len += 2 * this.options.navheight + this.options.navspacing }
                        if (this.options.navshowplaypause &&
                            !this.options.navshowplaypausestandalone) { len += len > 0 ? this.options.navspacing : 0;
                    ...
                    if (this.options.navshowbuttons) {
                        ...
                        if (this.options.navshowplaypause && !this.options.navshowplaypausestandalone) { len += len > 0 ? this.options.navspacing : 0;
                ...
                if (this.options.navthumbnavigationstyle != "auto") {
                    if (this.options.navmultirows) {
                        ...
                        if (this.options.navthumbnavigationstyle != "arrowinside") { $carLeftArrow.css({ display: "block" });
                            ...
                            if (this.options.navthumbnavigationstyle == "arrow") {
                                ...
                                if (this.options.navdirection == "horizontal")
                                    if (this.options.navmultirows) {
                                        ...
                                        $navContainer.css({ width: "100%", "margin-left": m + "px" })
                                    } else { var n = Math.floor(($nav.width() - 2 * m + this.options.navspacing) / (this.options.navwidth + this.options.navspacing)); if (n <= 0) n = 1;
                    ...
                    } else { $carLeftArrow.css({ display: "none" });
                        $carRightArrow.css({ display: "none" }); if (this.options.navdirection == "vertical") $bulletWrapper.css({ "margin-top": 0 });
                    ...
                    if (this.options.navstyle == "thumbnails" && this.options.navthumbresponsive) {
                        ...
                        if (this.options.navstyle == "thumbnails" && this.options.navshowfeaturedarrow) { bulletSize += this.options.navdirection == "vertical" ? this.options.navfeaturedarrowimagewidth : this.options.navfeaturedarrowimageheight;
                        ...
                        } else { $carLeftArrow.css({ left: this.options.navthumbnavigationstyle == "arrowoutside" ? "-" + this.options.navthumbnavigationarrowimagewidth + "px" : "0px", top: "0px", "margin-top": p + "px" });
            ...
            createStyle: function() {
                ...
                if (!this.options.isresponsive) { $(".amazingslider-space-" + this.id, this.container).css({ width: this.options.width, height: this.options.height });
                    this.container.css({ "width": this.options.width, "height": this.options.navdirection == "horizontal" && this.options.navmultirows ? "auto" : this.options.height }) } else { this.resizeSlider(); var instance = this;
                ...
                if (this.options.navstyle == "thumbnails" && this.options.navthumbstyle !=
                    "imageonly") { styleCss += ".amazingslider-nav-thumbnail-tite-" + this.id + " {" + this.options.navthumbtitlecss + "}";
                    styleCss += ".amazingslider-nav-thumbnail-tite-" + this.id + ":hover {" + this.options.navthumbtitlehovercss + "}"; if (this.options.navthumbstyle == "imageandtitledescription" || this.options.navthumbstyle == "textonly") styleCss += ".amazingslider-nav-thumbnail-description-" + this.id + " {" + this.options.navthumbdescriptioncss + "}" }
            ...
            createArrows: function() {
                ...
                $leftArrow.click(function() { instance.slideRun(-2) });
                ...
                $rightArrow.hover(function() { $(this).css({ "background-position": "right bottom" }) },
                    function() { $(this).css({ "background-position": "right top" }) });
                $rightArrow.click(function() { instance.slideRun(-1) });
                if (this.options.arrowstyle == "always") { $leftArrow.css({ display: "block" });
                    $rightArrow.css({ display: "block" }) } else {
                    ...
                    $(".amazingslider-slider-" + this.id, this.container).hover(function() {
                        ...
                        } else { $(".amazingslider-arrow-left-" + instance.id, instance.container).fadeIn();
                            $(".amazingslider-arrow-right-" + instance.id, instance.container).fadeIn() }
                    }, function() {
                        instance.arrowTimeout = setTimeout(function() {
                            if (ASPlatforms.isIE678()) { $(".amazingslider-arrow-left-" + instance.id, instance.container).hide();
                                $(".amazingslider-arrow-right-" + instance.id, instance.container).hide() } else {
                ...
                if (this.options.navdirection == "vertical") {
                    if (l == 0) {
                        ...
                        $(".amazingslider-car-left-arrow-" + this.id, this.container).data("disabled",
                            true)
                    } else { $(".amazingslider-car-left-arrow-" + this.id, this.container).css({ "background-position": "left top", cursor: "pointer" });
                        $(".amazingslider-car-left-arrow-" + this.id, this.container).data("disabled", false) }
                    if (l == $navContainer.height() - $bulletWrapper.height()) { $(".amazingslider-car-right-arrow-" + this.id, this.container).css({ "background-position": "right bottom", cursor: "" });
                        $(".amazingslider-car-right-arrow-" + this.id, this.container).data("disabled", true) } else {
                        $(".amazingslider-car-right-arrow-" + this.id,
                            this.container).css({ "background-position": "right top", cursor: "pointer" });
                        $(".amazingslider-car-right-arrow-" + this.id, this.container).data("disabled", false)
                    }
                } else {
                    if (l == 0) { $(".amazingslider-car-left-arrow-" + this.id, this.container).css({ "background-position": "left bottom", cursor: "" });
                        $(".amazingslider-car-left-arrow-" + this.id, this.container).data("disabled", true) } else {
                    ...
                    if (l == $navContainer.width() - $bulletWrapper.width()) { $(".amazingslider-car-right-arrow-" + this.id, this.container).css({ "background-position": "right bottom", cursor: "" });
                        $(".amazingslider-car-right-arrow-" + this.id, this.container).data("disabled", true) } else { $(".amazingslider-car-right-arrow-" + this.id, this.container).css({ "background-position": "right top", cursor: "pointer" });
                        $(".amazingslider-car-right-arrow-" + this.id, this.container).data("disabled", false) }
                }
            },
            createNav: function() {
                ...
                if (this.options.navstyle == "thumbnails") {
                    if (this.options.navthumbresponsive) {
                        ...
                        if (winW < this.options.navthumbsmallsize) { this.options.navwidth = this.options.navthumbsmallwidth;
                            ...
                            this.options.navthumbtitlewidth = this.options.navthumbsmalltitlewidth } else if (winW < this.options.navthumbmediumsize) {
                ...
                if (this.options.navstyle != "none") {
                    var $bullet;
                    for (i = 0; i < this.elemArray.length; i++) { $bullet = this.createNavBullet(i);
                    ...
                    $nav.mouseleave(function() { instance.pauseCarousel = false });
                    if (instance.options.navthumbnavigationstyle == "auto") $nav.mousemove(function(e) {
                        ...
                        $carLeftArrow.css({ overflow: "hidden", position: "absolute", cursor: "pointer", display: "none", width: instance.options.navthumbnavigationarrowimagewidth + "px", height: instance.options.navthumbnavigationarrowimageheight + "px", background: "url('" + instance.options.navthumbnavigationarrowimage + "') no-repeat left top" });
                        $carRightArrow.css({
                            overflow: "hidden",
                            position: "absolute",
                            cursor: "pointer",
                        ...
                        if (instance.options.navdirection == "vertical") { $carLeftArrow.css({ top: instance.options.navthumbnavigationstyle == "arrowoutside" ? "-" + instance.options.navthumbnavigationarrowimageheight + "px" : "0px", left: "0px", "margin-left": p + "px" });
                            $carRightArrow.css({ bottom: instance.options.navthumbnavigationstyle == "arrowoutside" ? "-" + instance.options.navthumbnavigationarrowimageheight + "px" : "0px", left: "0px", "margin-left": p + "px" }) } else {
                            $carLeftArrow.css({
                        ...
                        $carLeftArrow.click(function() { if (instance.options.navdirection == "vertical") instance.carMoveBottom();
                        ...
                        if (instance.options.navdirection == "vertical" && $bulletWrapper.height() > $navContainer.height() || instance.options.navdirection == "horizontal" && $bulletWrapper.width() > $navContainer.width()) { $carLeftArrow.css({ display: "block", "background-position": "left bottom", cursor: "" });
                        ...
                        if (instance.options.navthumbnavigationstyle == "arrowinside") {
                            ...
                            $nav.hover(function() {
                                if (instance.options.navdirection ==
                                    "vertical" && $bulletWrapper.height() > $navContainer.height() || instance.options.navdirection == "horizontal" && $bulletWrapper.width() > $navContainer.width()) { $carLeftArrow.fadeIn();
                                    $carRightArrow.fadeIn() }
                            }, function() { $carLeftArrow.fadeOut();
                    ...
                    if (instance.options.navdirection == "vertical") $nav.sliderTouchSwipe({ preventWebBrowser: true, swipeTop: function(data) { instance.carMoveTop() }, swipeBottom: function() { instance.carMoveBottom() } });
                        ...
                        swipeLeft: function(data) { instance.carMoveLeft() },
                        swipeRight: function() { instance.carMoveRight() }
                    });
                    this.container.bind("amazingslider.switch", function(event, prev, cur) { $("#amazingslider-bullet-" + instance.id + "-" + prev, instance.container)["bulletNormal" + instance.id]();
                ...
                if (this.options.navshowbuttons) {
                    ...
                    if (this.options.navshowarrow) {
                        ...
                        if (this.options.navarrowimage) $navLeft.css({ "background-image": "url('" + this.options.skinsfolder + this.options.navarrowimage + "')", "background-repeat": "no-repeat", "background-position": "left top" });
                        $navLeft.hover(function() { if (instance.options.navbuttonhighlightcolor) $(this).css({ "background-color": instance.options.navbuttonhighlightcolor }); if (instance.options.navarrowimage) $(this).css({ "background-position": "left bottom" }) },
                        ...
                        $navLeft.click(function() { instance.slideRun(-2) });
                    ...
                    if (this.options.navshowplaypause) {
                        ...
                        $navPlay.click(function() { instance.isPaused = false;
                            instance.loopCount = 0; if (!instance.videoPaused) instance.sliderTimeout.start();
                        ...
                        $navPause.click(function() { instance.isPaused = true;
                        ...
                        if (this.options.navshowplaypausestandalone && this.options.navshowplaypausestandaloneautohide) {
                            ...
                            this.$wrapper.hover(function() { if (instance.isPaused) { $navPlay.fadeIn();
                                        $navPause.css({ display: "none" }) } else { $navPlay.css({ display: "none" });
                                        $navPause.fadeIn() } },
                                function() { $navPlay.fadeOut();
                                    $navPause.fadeOut() })
                        } else { $navPlay.css({ display: instance.isPaused ? "block" : "none" });
                    ...
                    if (this.options.navshowarrow) {
                        ...
                        if (this.options.navarrowimage) $navRight.css({ "background-image": "url('" + this.options.skinsfolder + this.options.navarrowimage + "')", "background-repeat": "no-repeat", "background-position": "right top" });
                        $navRight.hover(function() { if (instance.options.navbuttonhighlightcolor) $(this).css({ "background-color": instance.options.navbuttonhighlightcolor }); if (instance.options.navarrowimage) $(this).css({ "background-position": "right bottom" }) },
            ...
            resizeNavBullets: function() {
                ...
                } else if (winW < this.options.navthumbmediumsize) { this.options.navwidth = this.options.navthumbmediumwidth;
                    this.options.navheight = this.options.navthumbmediumheight;
                    this.options.navthumbtitleheight = this.options.navthumbmediumtitleheight;
                    this.options.navthumbtitlewidth = this.options.navthumbmediumtitlewidth } else {
                ...
                $(".amazingslider-bullet-" + this.id, this.container).each(function() {
                    ...
                    if (instance.options.navthumbstyle != "textonly") {
                        $(".amazingslider-bullet-image-" +
                            instance.id, this).css({ width: instance.options.navimagewidth + "px", height: instance.options.navimageheight + "px" });
            ...
            createNavBullet: function(index) {
                ...
                $bullet.hover(function() {
                    ...
                    if (instance.options.navshowpreview) {
                        ...
                        if (instance.options.navdirection == "vertical") { var $navContainer = $(".amazingslider-nav-container-" + instance.id, instance.container);
                        ...
                        $(imgLoader).load(function() {
                            ...
                            if (instance.options.navdirection == "vertical") {
                                ...
                                if ($preview.is(":visible")) { $curImg.animate({ top: "0px" });
                                    $preview.stop(true, true).animate(p) } else { $curImg.css({ top: "0px" });
                                ...
                                if ($preview.is(":visible")) { $curImg.animate({ left: "0px" });
                                    $preview.stop(true, true).animate(p) } else { $curImg.css({ left: "0px" });
                                    $preview.stop(true, true).css(p).fadeIn() }
                            }
                        });
                    ...
                    if (instance.options.navshowpreview) { var $preview = $(".amazingslider-nav-preview-" + instance.id, instance.container);
                        $preview.delay(500).fadeOut() }
                });
                $bullet.click(function() { instance.slideRun($(this).data("index"), instance.options.playvideoonclickthumb) });
                if (this.options.navstyle == "bullets") {
                    ...
                    $.fn["bulletSelected" + this.id] = function() { $(this).css({ "background-position": "left bottom" });
                    ...
                    $.fn["bulletNormal" + this.id] = function() { $(this).css({ opacity: instance.options.navopacity, filter: "alpha(opacity=" + Math.round(100 * instance.options.navopacity) + ")" }); if (instance.options.navbordercolor) $(this).css({ "background-color": instance.options.navbordercolor }) };
                    ...
                    $.fn["bulletSelected" + this.id] = function() { $(this).css({ opacity: 1, filter: "alpha(opacity=100)" }); if (instance.options.navborderhighlightcolor) $(this).css({ "background-color": instance.options.navborderhighlightcolor });
                        instance.adjustNav(this) }
                }
                return $bullet
            },
            ...
            slideRun: function(index, playVideo, transEffect) {
                ...
                else { $(".amazingslider-video-wrapper-" + this.id, this.container).css({ display: "none" }).empty(); var slideDirection = true; if (index == -2) slideDirection = false;
                    else if (index == -1) slideDirection = true;
                    else if (index >= 0) slideDirection = this.curElem > savedCur ? true : false;
                    this.showImage(slideDirection, transEffect) }(new Image).src = this.elemArray[this.prevElem][ELEM_SRC];
                (new Image).src =
            ...
            showImage: function(slideDirection, transEffect) {
                ...
                $(imgLoader).load(function() {
                    ...
                    if (sliderRatio <
                        imgRatio)
                        if (instance.options.scalemode == "fit") { instance.curImageMaxWidth = sliderRatio / imgRatio * 100;
                            instance.curImageMarginLeft = 50 - instance.curImageMaxWidth / 2 } else instance.curImageMarginTop = -1 * (imgRatio - sliderRatio) * 50;
                    else if (instance.options.scalemode == "fit") instance.curImageMarginTop = (sliderRatio - imgRatio) * 50;
                    else { instance.curImageMarginLeft = -1 * (1 / imgRatio - 1 / sliderRatio) * 50;
                    ...
                    }, function() { instance.isAnimating = false;
                        instance.sliderTimeout.resume(true) }, function(timeoutid) { instance.transitionTimeout = timeoutid; if (!instance.isPaused && !instance.videoPaused && instance.elemArray.length > 1) instance.sliderTimeout.startandpause() });
                    ...
                    if (instance.elemArray[instance.curElem][ELEM_LINK]) {
                        ...
                        $swipeBox.unbind("click").bind("click",
                            function() {
                                if (instance.elemArray[instance.curElem][ELEM_LIGHTBOX]) { $(window).trigger("amazingslider.lightboxclicked", [instance.options.sliderid, instance.elemArray[instance.curElem][ELEM_ID], instance.elemArray[instance.curElem][ELEM_SRC], instance.elemArray[instance.curElem][ELEM_TITLE], instance.elemArray[instance.curElem][ELEM_LINK]]); if (!instance.isPaused) { instance.lightboxPaused = true;
                                        instance.isPaused = true;
                                        instance.sliderTimeout.pause() }
                                    instance.html5Lightbox.showItem(instance.elemArray[instance.curElem][ELEM_LINK]) } else {
            ...
            calcIndex: function(index) {
                ...
                } else if (index == -1) { this.prevElem = this.curElem;
                    this.curElem = this.nextElem;
                    this.nextElem = this.curElem + 1 >= this.elemArray.length ? 0 : this.curElem + 1 } else if (index >= 0) { this.curElem = index;
                    this.prevElem = this.curElem - 1 < 0 ? this.elemArray.length - 1 : this.curElem - 1;
        ...
        this.each(function() {
            ...
            for (var i = 0; i < searchstring.length; i++) { var keyvalue = searchstring[i].split("="); if (keyvalue && keyvalue.length == 2) { var key = keyvalue[0].toLowerCase(); var value = unescape(keyvalue[1]).toLowerCase(); if (value == "true") searchoptions[key] = true;
            ...
            var defaultOptions = {
                ...
                threedhorizontalwithzoom: { checked: true, effectdirection: 0, bgcolor: "#222222", perspective: 1E3, slicecount: 4, duration: 2200, scale: 0.8, scaleduration: 400, easing: "ease", fallback: "slice", scatter: 5, perspectiveorigin: "bottom" },
                fade: {
                    duration: 1E3,
                    easing: "easeOutCubic",
                    checked: true
                ...
                threedwithzoom: { checked: true, effectdirection: 0, bgcolor: "#222222", perspective: 1E3, slicecount: 5, duration: 2500, scale: 0.8, scaleduration: 400, easing: "ease", fallback: "slice", scatter: 5, perspectiveorigin: "right" },
                blinds: {
                    duration: 1500,
                    easing: "easeOutCubic",
                    checked: true,
                ...
                threedflipwithzoom: { duration: 2E3, easing: "ease-out", depth: 20, scale: 0.8, scaleduration: 400, checked: true, effectdirection: 0, bgcolor: "#222222", perspective: 1E3, slicecount: 5, fallback: "flipwithzoom", scatter: 4, perspectiveorigin: "bottom" },
            ...
            for (var i = 0; i < effects.length; i++) { var effectduration = effects[i] + "duration"; if (effectduration in this.options) this.options[effects[i]].duration = this.options[effectduration]; var effectfallback = effects[i] + "fallback"; if (effectfallback in this.options) this.options[effects[i]].fallback = this.options[effectfallback] }
...
(function($) { $.ASUpdateObject = function(op0, op1) { for (var key in op0)
...
(function($) {
    $.fn.amazingsliderTransition = function(id, $prev, $next, transition, callback, transitionStartCallback) {
        ...
        if (effect == "fade") { transitionStartCallback();
            $parent.css({ overflow: "hidden" });
            $next.show(); if ($prev && $prev.length) $prev.fadeOut(duration, easing, function() { $prev.remove();
                callback() });
            else callback() } else if (effect == "fadein") { transitionStartCallback();
            $parent.css({ overflow: "hidden" });
            $next.hide();
            $prev.insertBefore($next);
            $next.fadeIn(duration, easing, function() { $prev.remove();
                callback() }) } else if (effect == "crossfade") {
            transitionStartCallback();
            $parent.css({ overflow: "hidden" });
            $next.hide();
            if ($prev && $prev.length) $prev.fadeOut(duration / 2, easing, function() { $next.fadeIn(duration / 2, easing, function() { $prev.remove();
                    callback() }) });
            else callback()
        } else if (effect == "kenburns") {
            $parent.css({ overflow: "hidden" });
            $parent.kenburnsTransition(id, $prev, $next, $.extend({ duration: duration, easing: easing, direction: direction, imgmargintop: transition.imgmargintop, imgmarginleft: transition.imgmarginleft, imgmaxwidth: transition.imgmaxwidth }, transition[effect]), callback,
                transitionStartCallback)
        } else if (effect == "slide" || effect == "elastic") {
            transitionStartCallback();
            $parent.css({ overflow: "hidden" });
            if (direction) { $next.css({ left: "100%" });
                $next.animate({ left: "0%" }, duration, easing); if ($prev && $prev.length) $prev.animate({ left: "-100%" }, duration, easing, function() { $prev.remove();
                    callback() });
                else callback() } else {
                $next.css({ left: "-100%" });
                $next.animate({ left: "0%" }, duration, easing);
                if ($prev && $prev.length) $prev.animate({ left: "100%" }, duration, easing, function() { $prev.remove();
                    callback() });
                else callback()
            }
        } else if (effect == "slice") { $parent.css({ overflow: "hidden" });
            $parent.sliceTransition(id, $prev, $next, $.extend({ duration: duration, easing: easing, direction: direction, imgmargintop: transition.imgmargintop, imgmarginleft: transition.imgmarginleft, imgmaxwidth: transition.imgmaxwidth }, transition["slice"]), callback, transitionStartCallback) } else if (effect == "flip" || effect == "flipwithzoom") {
            $parent.css({ overflow: "hidden" });
        ...
        } else if (effect == "blocks") { $parent.css({ overflow: "hidden" });
            $parent.blocksTransition(id, $prev, $next, $.extend({ duration: duration, easing: easing, direction: direction, imgmargintop: transition.imgmargintop, imgmarginleft: transition.imgmarginleft, imgmaxwidth: transition.imgmaxwidth }, transition[effect]), callback, transitionStartCallback) } else if (effect == "threedtiles") {
            $parent.css({ overflow: "hidden" });
    ...
    $.fn.kenburnsTransition = function(id, $prev, $next, options, callback, transitionStartCallback) {
        var $parent = this;
        $parent.unbind("transitionFinished").bind("transitionFinished", function() { $parent.unbind("transitionFinished");
            $prev.remove();
            callback() });
        $next.hide();
        $prev.insertBefore($next);
        ...
        if (ASPlatforms.isIE6789()) {
            ...
            if (effect == "zoomout") { $next.css({ width: scale + "%", height: scale + "%", display: "block", opacity: 0 });
                $next.animate({ opacity: 1, width: "100%", height: "100%" }, options.duration, options.easing) } else { $next.css({ width: "100%", height: "100%", display: "block", opacity: 0 });
            ...
            var sliderRatio = h / w;
            if (sliderRatio < imgRatio) { h0 = originalwidth * sliderRatio;
                t0 = (originalheight - h0) / 2 } else { w0 = originalheight / sliderRatio;
            ...
            if (effect == "zoomout") {
                ...
                (function asDrawKenBurnsZoomOut() {
                    ...
                    switch (direction) {
                        case 0:
                            context.drawImage(image, l0, t0, w0, h0, w - scale * w, h - scale * h, scale * w, scale * h); break;
                        case 1:
                            context.drawImage(image, l0, t0, w0, h0, 0, h - scale * h, scale * w, scale * h); break;
                        case 2:
                            context.drawImage(image, l0, t0, w0, h0, w - scale * w, 0, scale * w, scale * h); break;
                        default:
    ...
    $.fn.sliceTransition = function(id, $prev, $next, options, callback, transitionStartCallback) {
        ...
        for (i = 0; i < options.slicecount; i++) {
            ...
            if (Math.abs(options.imgmarginleft) > 0) { $img.css({ "margin-left": options.slicecount * options.imgmarginleft + "%" });
                $img.css({ "max-width": options.slicecount * options.imgmaxwidth + "%", left: "-" + sliceW * i + "px", width: options.slicecount * options.imgmaxwidth + "%" }) } else {
                $img.css({
        ...
        slices.each(function() {
            ...
            switch (effect) {
                case "up":
                    slice.css({ top: "", bottom: "0%", height: "0%" }); break;
                case "down":
                    slice.css({ top: "0%", height: "0%" }); break;
                case "updown":
                    if (index % 2 == 0) slice.css({ top: "0%", height: "0%" });
                    else slice.css({ top: "", bottom: "0%", height: "0%" }); break }
            setTimeout(function() { slice.animate({ height: "100%" }, duration, options.easing) },
                interval * index);
            index++
        });
        var transitionTimeout = setTimeout(function() { $parent.trigger("transitionFinished") }, options.duration);
    ...
    $.fn.blindsTransition = function(id, $prev, $next, options, callback, transitionStartCallback) {
        ...
        for (i = 0; i < options.slicecount; i++) {
            ...
            } else { $img.css({ "margin-top": options.slicecount * options.imgmargintop + "%" });
                $img.css({ "max-width": options.slicecount * 100 + "%", left: "-" + sliceW * i + "px", width: options.slicecount * 100 + "%" }) }
            $imgSlice.append($img);
        ...
        index = 0;
        slices.each(function() { var slice = $(this); var target; if (!options.direction) { slice.css({ left: "", right: "-100%" });
                target = { right: "0%" } } else { slice.css({ left: "-100%" });
                target = { left: "0%" } }
    ...
    $.fn.threedTransition = function(id, $prev, $next, options, callback, transitionStartCallback) {
        ...
        for (i = 0; i < options.slicecount; i++) {
            ...
            if (Math.abs(options.imgmarginleft) > 0) { $nextImg.css({ "margin-left": options.slicecount * options.imgmarginleft + "%" });
                $nextImg.css({ "max-width": options.slicecount * options.imgmaxwidth + "%", left: "-" + sliceW * i + "px", width: options.slicecount * options.imgmaxwidth + "%" }) } else {
            ...
            } else { $curImg.css({ "margin-top": options.slicecount * options.previmgmargintop + "%" });
        ...
        if (totalDuration < 0) { scalePrepare = 0;
            scaleDuration = 0;
            totalDuration = options.duration }
        var interval =
            totalDuration / 2 / options.slicecount;
        var duration = totalDuration / 2;
        cubes.each(function() { $(this).css(ASPlatforms.applyBrowserStyles({ "transform-style": "preserve-3d", "backface-visibility": "hidden" }));
        ...
        if (scaleDuration > 0) {
            ...
            setTimeout(function() { cubes.css(ASPlatforms.applyBrowserStyles({ "transform": "scale(" + options.scale + ") translateZ(-" + dist + "px" + ")" })) }, scalePrepare)
        }
        index = 0;
        cubes.each(function() {
            var cube = $(this);
            ...
            setTimeout(function() {
                ...
                else cube.css(ASPlatforms.applyBrowserStyles({ "transition-timing-function": options.easing, "transition-duration": duration + "ms", "transform": "translateZ(-" + dist + "px" + ") rotateX(" + (options.direction ? "-89.99" : "89.99") + "deg)" }));
    ...
    $.fn.threedHorizontalTransition = function(id, $prev, $next, options, callback, transitionStartCallback) {
        ...
        for (i = 0; i < options.slicecount; i++) {
            ...
            } else { $nextImg.css({ "margin-top": options.imgmargintop + "%" });
                $nextImg.css({ "max-width": "100%", top: "-" + sliceH * i + "px", width: "100%" }) }
            var $nextImgSlice = $("<div class='amazingslider-img-slice-" + id + " ' style='display:block;position:absolute;left:0%;top:0%;width:100%;height:100%;overflow:hidden;outline:1px solid transparent;background-color:" + options.bgcolor + ";'></div>");
            $nextImgSlice.append($nextImg);
            var $curImg = $("img", $prev).clone();
            if (Math.abs(options.previmgmarginleft) > 0) { $curImg.css({ "margin-left": options.previmgmarginleft + "%", height: "auto" });
                $curImg.css({ "max-width": options.previmgmaxwidth + "%", top: "-" + sliceH * i + "px", width: options.previmgmaxwidth + "%" }) } else { $curImg.css({ "margin-top": options.previmgmargintop + "%" });
        ...
        cubes.each(function() {
            ...
            setTimeout(function() {
                ...
                else cube.css(ASPlatforms.applyBrowserStyles({ "transition-timing-function": options.easing, "transition-duration": duration + "ms", "transform": "translateZ(-" + dist + "px" + ") rotateY(" + (options.direction ? "89.99" : "-89.99") + "deg)" }));
    ...
    $.fn.threedFlipTransition = function(id, $prev, $next, options, callback, transitionStartCallback) {
        ...
        for (i = 0; i < options.slicecount; i++) {
            ...
            } else { $nextImg.css({ "margin-top": options.slicecount * options.imgmargintop + "%" });
            ...
            $nextImgSlice.append($nextImg);
            var $curImg = $("img", $prev).clone();
            if (Math.abs(options.previmgmarginleft) > 0) { $curImg.css({ "margin-left": options.slicecount * options.previmgmarginleft + "%" });
                $curImg.css({ "max-width": options.slicecount * options.previmgmaxwidth + "%", left: "-" + sliceW * i + "px", width: options.slicecount * options.previmgmaxwidth + "%" }) } else {
        ...
        $parent.unbind("transitionFinished").bind("transitionFinished", function() {
            ...
            $prev.remove();
            $next.show();
            setTimeout(function() { $cubeWrapper.remove() },
                100);
            callback()
        });
        var scalePrepare = options.scale == 1 ? 0 : 12;
    ...
    $.fn.threedtilesTransition = function(id, $prev, $next, options, callback, transitionStartCallback) {
        ...
        for (i = 0; i < options.rowcount; i++)
            for (j = 0; j < options.columncount; j++) {
                ...
                } else { $nextImg.css({ "margin-top": options.columncount * options.imgmargintop + "%" });
                ...
                var $curImg = $("img", $prev).clone();
                if (Math.abs(options.previmgmarginleft) > 0) { $curImg.css({ "margin-left": options.columncount * options.previmgmarginleft + "%" });
                    $curImg.css({ "max-width": options.columncount * options.previmgmaxwidth + "%", left: "-" + blockW * j + "px", top: "-" + blockH * i + "px", width: options.columncount * options.previmgmaxwidth + "%", height: "auto" }) } else {
        ...
        var duration = totalDuration /
            2;
        blocks.each(function() { $(this).css(ASPlatforms.applyBrowserStyles({ "transform-style": "preserve-3d", "backface-visibility": "hidden" }));
            $(this).css(ASPlatforms.applyBrowserStyles({ "transition-property": "transform" }, true));
            $(this).css(ASPlatforms.applyBrowserStyles({ "transform": "translateZ(-" + options.depth / 2 + "px" + ")" })) });
        $prev.hide();
        index = 0;
        ...
        blocks.each(function() { var block = $(this);
    ...
    $.fn.tilesTransition = function(id, $prev, $next, options, callback, transitionStartCallback) {
        ...
        for (i = 0; i < options.rowcount; i++)
            for (j = 0; j < options.columncount; j++) {
                var $nextImg = $("img", $next).clone();
                if (Math.abs(options.imgmarginleft) > 0) { $nextImg.css({ "margin-left": options.columncount * options.imgmarginleft + "%" });
                    $nextImg.css({ "max-width": options.columncount * options.imgmaxwidth + "%", left: "-" + blockW * j + "px", top: "-" + blockH * i + "px", width: options.columncount * options.imgmaxwidth + "%", height: "auto" }) } else {
                ...
                } else { $curImg.css({ "margin-top": options.columncount * options.previmgmargintop + "%" });
        ...
        blocks.each(function() {
            var block = $(this);
            setTimeout(function() { block.find(".amazingslider-img-block-next").css(ASPlatforms.applyBrowserStyles({ "transition-timing-function": options.easing, "transition-duration": duration + "ms", "transform": "rotateY(0deg)" }));
                    block.find(".amazingslider-img-block-current").css(ASPlatforms.applyBrowserStyles({ "transition-timing-function": options.easing, "transition-duration": duration + "ms", "transform": "rotateY(180deg)" })) }, scaleDuration +
    ...
    $.fn.blocksTransition = function(id,
        ...
        for (i = 0; i < options.rowcount; i++)
            for (j = 0; j < options.columncount; j++) {
                ...
                } else { $img.css({ "margin-top": options.columncount * options.imgmargintop + "%" });
    ...
    $.fn.shuffleTransition = function(id, $prev, $next, options, callback, transitionStartCallback) {
        ...
        for (i = 0; i < options.rowcount; i++)
            for (j = 0; j < options.columncount; j++) {
                ...
                if (Math.abs(options.imgmarginleft) > 0) { $imgNext.css({ "margin-left": options.columncount * options.imgmarginleft + "%" });
                    $imgNext.css({ "max-width": options.columncount * options.imgmaxwidth + "%", left: "-" + blockW * j + "px", top: "-" + blockH * i + "px", width: options.columncount * options.imgmaxwidth + "%", height: "auto" }) } else {
                ...
                if (Math.abs(options.imgmarginleft) > 0) { $imgPrev.css({ "margin-left": options.columncount * options.imgmarginleft + "%" });
                    $imgPrev.css({ "max-width": options.columncount * options.imgmaxwidth + "%", left: "-" + blockW * j + "px", top: "-" + blockH * i + "px", width: options.columncount * options.imgmaxwidth + "%", height: "auto" }) } else {
        ...
        index = 0;
        blocksNext.each(function() { var block = $(this); var row = Math.floor(index / options.columncount); var col = index % options.columncount; var posL = Math.random() * (distR - distL) + distL; var posT = Math.random() * (distB - distT) + distT;
        ...
        var transitionTimeout =
            setTimeout(function() { $parent.trigger("transitionFinished") }, options.duration);
        transitionStartCallback(transitionTimeout)
    }
})(jQuery);
(function($) {
...
if (typeof amazingsliderObjects === "undefined") var amazingsliderObjects = new function() { this.objects = [];
    this.addObject = function(obj) { this.objects.push(obj) };
    this.loadNext = function(id) { for (var i = 0; i < this.objects.length; i++)
